...
with danem as
(SELECT
  jca.title as job ,jca.description,jca.salary,jca.insert_date,concat(jca.location_region,", ",jca.location_country) as location, {{dbt_utils.generate_surrogate_key(['jca.link'])}} as link_id,
  COALESCE(
...
where jca.job_category_class in ("Big Data","ERP &amp; CRM","Télécommunications","Systèmes / Réseaux et Télécoms","Cybersécurité","Business Intelligence","Développement Web &amp; Mobile")
 {% if is_incremental() %}
  and jca.insert_date >= (select coalesce(max(insert_date), '1900-01-01') from {{ this }})
{% endif %}
),
stg_job as (
