...
select 
to_hex(md5(cast(coalesce(cast(title as string), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(skill as string), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(location as string), '_dbt_utils_surrogate_key_null_') as string))) as job_id,
title, 
